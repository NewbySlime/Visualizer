...
bool displayHandler::_change_list(size_t contentLayerdec, int32_t additionN){
  ...
  if(additionN != 0){
    size_t newIndex = _llListInteract->_current->listIndex+additionN;
    if(newIndex < _llListInteract->_current->listinteract.size() && newIndex >= 0){
      _txt2 = &_llListInteract->_current->listinteract[_llListInteract->_current->listIndex];

      _llListInteract->_current->listIndex += additionN;
      _txt1 = &_llListInteract->_current->listinteract[_llListInteract->_current->listIndex];
      return true;
    }
  }
...
void displayHandler::back_tomainlist(){
  if(!_currentlyTransitioning && (_nlayersContent != 0 || _llListInteract->_current->listIndex != _mainpos)){
    ...
    _txt2 = &__dmpint;
    if(_nlayersContent > 0){
      _change_list(_nlayersContent, 0);
    }

    _llListInteract->_current->listIndex = _mainpos;
    _txt1 = &_llListInteract->_current->listinteract[_mainpos];
  }
}
