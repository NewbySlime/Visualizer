...
int main(){
  ...
  if(_intinput < n && _intinput >= 0){
    ...
    if(!FAILED(errcode)){
      ...
      float *fData = new float[minWindowSize*2];
      ...
      while(true){
        ...
        while(packetLength > 0){
          ...
          int addedlen = min(numFramesAvailable, minWindowSize - currentWindowSize);
          for(int in = 0; in < (addedlen*2); in++){
            fData[in+currentWindowSize] = newfdata[in];
          }

          currentWindowSize += addedlen;
          
          if(currentWindowSize >= minWindowSize){
            int freqbinlen = minWindowSize/2;
            float *freqbinarr = new float[freqbinlen];
            for(int f_i = 0; f_i < freqbinlen; f_i++){
              std::pair<float, float> ri_pair(0.f, 0.f);

              // for now calculate one of the channels
              for(int frames_i = 0; frames_i < minWindowSize; frames_i++){
                add_pair(ri_pair, DFT_partFunction(f_i, frames_i, minWindowSize, fData[frames_i*2]));
              }
              ...
              freqbinarr[f_i] = magnitude_pair(ri_pair)/minWindowSize;
            }

            //printf("freqbinlen %d\n", freqbinlen);
            freqbinlen = freqbinlen*((float)maxFrequency/pwfx->nSamplesPerSec);
            //printf("freqbinlen %d\n", freqbinlen);
            ...
            if(connect(s, (sockaddr*)localHostAddress, sizeof(*localHostAddress)) < 0){
              ...
              int data_i = 0;
              float *dataarr = (float*)malloc(sizeof(float) * arrsize);
          ...
          /*
          std::ofstream datfile; datfile.open("./freqlog.dat", datfile.binary);
          datfile.write(reinterpret_cast<char*>(&freqbinlen), sizeof(int));
          datfile.write(reinterpret_cast<char*>(freqbinarr), sizeof(float) * freqbinlen);
          datfile.close();
          */
