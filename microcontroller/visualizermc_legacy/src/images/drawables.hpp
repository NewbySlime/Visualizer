...
class draw_multiplelist: public draw_text{
  public:
    typedef void (*OnAcceptCallback)(int idx);

  private:
    void useText(const char *txt1, const char *txt2){draw_text::useText(txt1, txt2);}
    void useText1(const char *txt){draw_text::useText1(txt);}
    void useText2(const char *txt){draw_text::useText2(txt);}

  protected:
    std::pair<const char*, const char*> *_liststr;
    size_t _liststrlen;

    int _curridx;
    int _beforeidx;

    bool _stillTransitioning = true;

    OnAcceptCallback _cb1 = NULL;

    void _setCurrentIndex(int idx);
  ...
  public:
    ...
    void onDraw(drawable_cbparam &param);
    void passInput(sensor_actType act);

    void setCurrentIndex(int idx);
    void setStr(int idx, const char *str1, const char *str2);
    void setStr1(int idx, const char *str);
    void setStr2(int idx, const char *str);
    void setCallbackOnAccept(OnAcceptCallback cb);
