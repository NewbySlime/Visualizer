  ...
  if(queue_edit.size()){
    ...
    switch((_queue_enum)_qtype){
      ...
      break; case read:{
        ...
        int _readsize = _fi->high_bound-_fi->low_bound;
        if(_rq->datasize > 0)
          _readsize = min(_rq->datasize, _readsize);

        DEBUG_PRINT("_readsize %d\n", _readsize);
        bound_storage->bufReadAsync(_fi->low_bound, _rq->data, _readsize, __on_donewr, this);
        _rq->_q = done_r;
      }
...
fs_error file_system::read_file(uint16_t file_id, char *data, fs_readcb cb, void *obj, int _readsize){
  if(!bound_storage)
...
bool file_system::is_storage_bound(){
  return bound_storage != NULL;
}
