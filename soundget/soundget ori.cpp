...
void SoundGet::_ThreadUpdate(){
  ...
  BYTE *bdata = new BYTE[WINDOWSIZE*samplesPerFrames*bytePerSample];
  while(isListening){
    BYTE *pData;
    ...
    while(packetLength > 0 && isListening){
      ...
      int addedlen = mmin(numFramesAvailable, (UINT32)WINDOWSIZE - currentWindowSize);
      memcpy(&bdata[currentWindowSize*samplesPerFrames*bytePerSample], pData, addedlen*samplesPerFrames*bytePerSample);
      ...
      if(currentWindowSize >= WINDOWSIZE || packetLength <= 0){
        ...
        for(int f_i = 0; f_i < freqbinlen; f_i++){
          for(int c_i = 0; c_i < samplesPerFrames; c_i++){
            vec2d ri_pair;
            for(int frames_i = 0; frames_i < currentWindowSize; frames_i++)
              ri_pair += DFT_partFunction(
                f_i,
                frames_i,
                currentWindowSize,
                _GetFloatInTime(&bdata[(frames_i*samplesPerFrames+c_i)*bytePerSample])
              );
      ...
      if(addedlen < numFramesAvailable){
        currentWindowSize = numFramesAvailable-addedlen;
        memcpy(bdata, &pData[addedlen*samplesPerFrames*bytePerSample], currentWindowSize*samplesPerFrames*bytePerSample);
      }
